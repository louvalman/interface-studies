...
- **`--step`: geometry that comes out of the copy.** The other shapes are
  ...
  **A mask is an image, so its arcs get whatever antialiasing the gradient
  ...
  block, and a circle-fit RMS of 0.20 device px against 0.056. Ramping over a
  whole pixel instead — `--…-step-feather`, centred on the radius so the arc
  stays put — turns that around completely. Every arc now measures fully
  antialiased at 2x and 3x, with a circle-fit RMS of 0.046 device px against
  0.115 for the `border-radius` control, and tangent to both the band's edge
  and the tread to within 0.2 device px. The mask corners are now the *rounder*
  ones on the block.

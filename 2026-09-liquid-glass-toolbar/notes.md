...
And once it is alone in the bar, the lit pill has to stop being a pill. Its job
in a row is to say which of five items you are on; with the other four gone it
is saying that about the only thing there, and what you actually see is its rim
tracing the shell's rim five pixels inside it — two stadiums drawing the same
shape, which is the one thing this component is not. The surface changes shape;
it does not grow a second edge. So in that state the pill hands its edge and
its body back to the shell and the field sits directly on the glass. The
trailing allowance goes with them: `--label-trail` balances an icon's bearing
against a word, and there is no word here — left on, it put the field 7px from
one end of the shell and 13px from the other. The shadows are written
transparent rather than `none` so the change still interpolates, which is why
the base declaration is transparent too.
